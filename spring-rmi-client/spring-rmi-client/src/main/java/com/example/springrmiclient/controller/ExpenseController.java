...
//	
	...
	public List<Expense> getExpenses() throws NumberFormatException, ParseException{
		...
		Expense expense;
		result = helloWorldRMI.getRecords();
		for (int i = 0; i < result.size(); i++) {
			cat = result.get(i);
//			String[] splittedCategory = cat.get(4).split(" ");
//			String[] splittedUser = cat.get(5).split(" ");
			List<String> categoryList = helloWorldRMI.getActivity(Long.parseLong(cat.get(4)));
			Activity activity = new Activity(Long.parseLong(categoryList.get(0)), categoryList.get(1));
	...
	public Expense createExpense(@Valid @RequestBody Expense expense) throws URISyntaxException{
		...
		helloWorldRMI.createRecord(list);
		  return expense; 
	...
	public List<Activity> activities() {
		...
		Activity activity;
		result = helloWorldRMI.activities();
		for (int i = 0; i < result.size(); i++) {
			cat = result.get(i);
			activity = new Activity(Long.parseLong(cat.get(0)), cat.get(1));
...
//	}
	...
	public Activity createCategory(@Valid @RequestBody Activity activity) throws URISyntaxException{
		System.out.println("Client " + activity.toString());
	
		String[] arr = new String[2];
		arr[0] = activity.getId().toString();
		arr[1] = activity.getName();
		
	  helloWorldRMI.createActivity(arr);
	  return activity; 
//	
	}
	...
	public void updateCategory(@Valid @RequestBody Activity activity){
		helloWorldRMI.updateActivity(activity);
	}
	...
	public ResponseEntity<?> deleteCategory(@PathVariable Long id){
		helloWorldRMI.deleteActivity(id);
		return ResponseEntity.ok().build();
	}
}
